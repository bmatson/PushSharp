...
namespace PushSharp.Core
{
	...
	public abstract class PushServiceBase : IPushService
	{
		...
		private void CheckScale(object state = null)
		{
            ...
            try
            {
                synch = Interlocked.CompareExchange(ref scaleSynch, 1, 0);
                if (synch == 0)
                {
        ...
        public TimeSpan AverageQueueWaitTime
        {
			...
			{ 
				...
				lock (measurements)
				{
                    ...
                    //if we have < 20 measurements, just return 0, we don't have enough traffic to scale up.
